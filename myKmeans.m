...
disp(data_max);
data_diff = bsxfun(@minus, data_max, data_min);
...
while pos_diff > 0.0
  ...
  for d = 1 : length( data(:, 1) );
    ...
    min_diff = bsxfun(@minus, data( d, :) , centroid( 1,:) );
    min_diff = sqrt(trace(min_diff * min_diff'));
    curAssignment = 1;

    for c = 2 : nbCluster;
      diff2c = bsxfun(@minus, data( d, :) , centroid( c,:) );
      diff2c = sqrt(trace(diff2c * diff2c'));
      if( min_diff >= diff2c)
        curAssignment = c;
        min_diff = diff2c;
      end
  ...
  for d = 1: length(assignment);
    centroid( assignment(d),:) = bsxfun(@plus, centroid( assignment(d),:),data(d,:));
    pointsInCluster( assignment(d), 1 ) = bsxfun(@plus, pointsInCluster( assignment(d), 1 ), 1) ;
  end
  ...
  %stoppingCriterion
  pos_diff = sum (sum( bsxfun(@minus,centroid, oldPositions).^2 ) );
